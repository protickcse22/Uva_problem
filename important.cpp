...
int main()
{
    ...
    for (int i = 1; s[i]; i++)
    {
        ...
        else
        {
            ...
            if (s[normalized[i] - 1] == ')' && normalized[ normalized[i] - 1 ])   leftmost[i] = leftmost[ normalized[i] - 1 ];
            else                                                                leftmost[i] = normalized[i];

            int len = (i - leftmost[i] + 1);

            if (len == mx)      cnt++;
            else if (len > mx)  mx = len, cnt = 1;
        }
    }
